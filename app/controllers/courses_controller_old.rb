class CoursesController < ApplicationController
  ...
  def create
      ...
      cst = params[:create_course][:status]
      cl = params[:create_course][:level]

      @course = Course.find_by(ccode: cc)
      if @course
        flash[:error] = "This course already exists"
      else
        @params = {ccode:cc, ctitle:ct, units:cu, level:cl, semester:cs, status:cst}

        test = cc.blank? || ct.blank? || cs.blank? || cu.blank? || cl.blank? ||cst.blank?
        if test
...
def course_create_params
    params.require(:create_course).permit(:ccode, :ctitle, :unit, :semester, :level)
end
